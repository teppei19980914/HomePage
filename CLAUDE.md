...
- **ブログ記事ファイル名**: `YYYYMMDD-slug.md`（例: `20260418-failure-science.md`）。date は既存最新日付から2〜4日後を目安に設定（無理のない頻度を優先。同日複数投稿は不可）
- **書き溜め運用可**: まとめて執筆する場合は複数記事の date を数日ずつずらして設定してよい（`BlogCalendar` は未来日付を「公開予定」表示する仕組みに対応済み）
- **ブログ記事の読了時間**: 3〜5分が目標。3分未満は深掘り、5分超は分割を検討
- **ブログ記事 CRUD 時のカレンダー点検**: `src/content/blog/{ja,en}/*.md` を追加・更新・削除した場合は必ず `npm test -- blog-calendar` を実行し、カレンダー（`src/components/BlogCalendar.astro`）に正しく反映されることを自動テストで確認する。`assertAllPostsCovered` と「同日複数投稿禁止」ルールが強制される
...
- **記事の設計から執筆まで一貫対応（例外: ユーザーから明示的に依頼があった場合のみ）**

**Claude Code が実施しない作業**:
- ホームページの実装修正・コード変更
- 記事の作成・投稿（**原則ユーザーが担当**。上記の例外依頼時のみ Claude Code が代行）
- コミット & プッシュ
...
**記事執筆代行時のルール**（例外運用）:
- ja → en の順で両言語のドラフトを作成
- SEO 基準（title 30-60 / description 120-160 / 読了時間 3-5 分 / 内部リンク 3 件以上）を満たすこと
- 作成した記事はユーザーがレビューし採否を判断する

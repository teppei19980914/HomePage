---
title: "たすきば 開発メンバー募集（募集終了）｜AI業務管理秘書の開発コミュニティだった記録"
description: "AI業務管理秘書「たすきば」の開発メンバー募集ページ（アーカイブ）。サービスは運用を終了し、開発メンバーの募集も終了しています。TypeScript・Next.jsで心理的安全性を大切にしたコミュニティ運営の記録として公開しています。ソースコードはGitHubで公開中です。"
...
> **【重要】たすきばは運用を終了しており、開発メンバーの募集も終了しています。** 以下は稼働当時の募集内容の記録です。ソースコードは [GitHub リポジトリ](https://github.com/teppei19980914/BusinessManagementPlatform) にてポートフォリオとして公開を継続しています。
...
| 自動テスト | 4,800 件超（Vitest 単体 4,580 超 + Playwright E2E 220 超） |
...
<details class="faq-item">
<summary>技術スタック（2026 年 7 月時点の構成）</summary>

参考までに、現在のたすきばで使っている技術をひと通りご紹介します。
**ここに書かれている技術を「知っていなくても」「触ったことがなくても」全く問題ありません**。むしろ「初めて触る技術が多い」のは、ご自身の学びの場として大きなチャンスだと思います。

> 各バージョン・スタックは継続的に更新されています。本ページの内容は **2026 年 7 月上旬時点のスナップショット** としてご覧ください。

> 興味のある領域のタイトルをクリックすると、詳細が開きます。

<details class="faq-item">
<summary>0. コードベースの規模（2026 年 7 月上旬時点）</summary>

「小さなプロジェクト」と言っても、実際どのくらいの規模なのか、数字でお見せします（リポジトリの実測値）。

| 指標 | 規模 |
|---|---|
| アプリ本体コード（TypeScript / TSX、テスト除く） | 約 133,000 行 / 664 ファイル |
| 単体テストコード（Vitest） | 約 69,000 行 / 311 ファイル・**4,580 ケース超** |
| E2E テストコード（Playwright） | 約 7,200 行 / 38 ファイル・**220 ケース超** |
| API エンドポイント（Route Handlers） | 185 本 |
| 画面（ページ） | 58 画面 |
| データベース定義（Prisma schema） | 57 モデル / マイグレーション 92 本 |
| ドキュメント | 220 ファイル（Markdown） |

> **行数の内訳**：たすきば本体のコード（テスト除く）が **約 133,000 行**、それとは別に存在する **単体テストコード** が **約 69,000 行**、合わせて **約 202,000 行**（本体 133,000 + 単体テスト 69,000）です。本体に対して単体テストは **約 52%** にあたる量を書いており、「テストファースト・品質重視」が数字にも表れています。各数値は開発が進むにつれ増えるため、おおよその目安としてご覧ください。
...
| 技術 | バージョン | 役割 |
|---|---|---|
| **Next.js** (App Router) | 16.2.9 | フロントとサーバを統合した React フレームワーク。Turbopack で高速ビルド |
| **React** | 19.2.7 | UI を「部品（コンポーネント）」単位で組み立てる仕組み |
...
| **pg (node-postgres)** | 8.22 | PostgreSQL クライアント |
...
| **Anthropic Claude** (@anthropic-ai/sdk) | 0.105 | LLM 呼び出し（自動タグ抽出など） |
...
| 技術 | バージョン | 役割 |
|---|---|---|
| **Vitest** | 4.x | 単体テスト（現在 **4,580 ケース超**） |
| **Playwright** | 1.61 | E2E テスト（ブラウザ自動操作、**220 ケース超**） |
...
参加するかどうかを、**いま一気に決める必要はありません**。ここから先は、私と一緒に、少しずつ距離を縮めていきましょう。気が向いたステップから、あなたのペースでどうぞ。
...
コードを書きたい、テストを試したい、ドキュメントを直したい、まずは観察したい —— どんなことでも大歓迎です。**「最初の一歩」は、私と一緒に設計しましょう。** ひとりで考え込まなくて大丈夫です。Discussions の **Ideas（やってみたいこと）** カテゴリに一言投げていただければ、私がすぐ拾って、一緒に進む道を考えます。
...
> 急かしません。Step 2 でいったん止まる方も、Step 3 で時間を置く方も、大丈夫。あなたのペースに、私のほうが合わせます。

<details class="faq-item">
<summary>メンバーになったら — 最初の1か月のおすすめの過ごし方</summary>

「アサインされたら、まず何をすればいいんだろう？」 —— ここは、**私と一緒に歩く最初の1か月** のイメージです。ノルマでも義務でもありません。「こんなふうに始めると、たすきばと仲良くなりやすいですよ」という、私からのおすすめの道案内だと思ってください。

最初の1か月は、いきなり大きな機能に挑むよりも、**たすきばを実際に一緒に触りながら、サービス全体と開発の流れに少しずつ慣れていく** 時間にしましょう。目安は **週 10 時間ほど（月 40 時間ほど）**。下の小さなループを何周かするうちに、「メンバーになったら何をやっていくのか」が、自然と体に馴染んでいくはずです（忙しい月は無理しなくて大丈夫。そこも一緒に調整しましょう）。

1. **たすきフクロウ（アプリ内 AI チャット）に聞いてみる** — 画面を触っていて「これ何だろう？」と思ったら、まずその場で AI に質問。気軽に、たくさん聞いて OK です
2. **AI がうまく答えられなかったことを 1 つ拾う** — すっきり答えが返ってこなかった話題を、ひとつ選びます。**ここが、私たちで直すと価値が生まれる「お宝の場所」** です
3. **サーバ側のロジックを、一緒にのぞいてみる** — その機能が裏でどう動いているのか、コードを読んで確かめます。詰まったら遠慮なく Discussions で呼んでください。一緒に読み解きましょう
4. **ドキュメントを最新にして、FAQ・使い方ガイドを育てる** — 分かったことを書き残して、次に入ってくる人のために道をならします

このループの嬉しいところは、**研修がそのまま本物の貢献になる** こと。あなたが FAQ や使い方ガイドを 1 つ増やすたびに、**たすきフクロウの回答もそのぶん賢くなります**。あなたの「分からなかった」が、次に入ってくる仲間やユーザの「分かった！」に変わっていく —— その積み重ねを、一緒に楽しんでいけたら嬉しいです。

ひとりで抱え込む必要は、まったくありません。迷ったら、いつでも [GitHub Discussions](https://github.com/teppei19980914/BusinessManagementPlatform/discussions) で声をかけてください。私も一緒に走ります。

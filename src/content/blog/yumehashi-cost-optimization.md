...
まず Firestore の課金軸をおさらいします（東京リージョン）。

| 課金軸 | 単価 | 無料枠 |
|---|---|---|
| 読み取り | $0.06 / 100K ops | 50,000 ops / 日 |
| 書き込み | $0.18 / 100K ops | 20,000 ops / 日 |
| ストレージ | $0.18 / GB / 月 | 1 GB |
| 下り帯域 | $0.12 / GB | 10 GB / 月 |

重要なのは **書き込みは「ops 回数」で課金され、ドキュメントサイズには依存しない** ことです。1 KB の書き込みでも 100 KB の書き込みでも、課金上は同じ「1 write」です。

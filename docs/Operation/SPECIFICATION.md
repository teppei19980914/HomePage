...
- 一覧: `order` 昇順で表示。ステータスバッジ（Active/Beta/Archived/Suspended/Ended）
- 詳細: frontmatter + Markdown 本文。「体験する →」（frontmatter `url`）「GitHub」（`repo`）ボタン（任意）
- **構造化データ（SoftwareApplication）**: `url` を持ち `status !== "ended"` の製品（＝実際に稼働中のアプリ）の詳細ページに `BaseLayout` の `extraJsonLd` 経由で schema.org `SoftwareApplication` JSON-LD を出力。`applicationCategory` は slug に `tasukiba` を含む場合 `BusinessApplication`、それ以外は `ProductivityApplication`。`offers`（`price: "0"` / `JPY`）は `status !== "suspended"` の製品のみ付与（受付停止中は無料オファーを宣言しない）。運用終了 (`ended`) の製品は実在しないアプリとして誤認させないため JSON-LD 自体を出力しない。検索結果でのリッチリザルト露出を高める目的
- `status: "suspended"` のとき: 詳細ページに「新規受付停止中」のお知らせバナーを表示し、「体験する」ボタンを非活性表示（クリック不可）に置換。GitHub ボタンは引き続き利用可能
- `status: "ended"` のとき: 詳細ページに「運用終了」のお知らせバナー（i18n `product.endedNotice`）を表示し、「体験する」ボタンを非活性表示（`product.endedLink`）に置換。GitHub ボタンは引き続き利用可能で、運用終了製品はソースコードの参照先として GitHub リポジトリへの導線を維持する。サーバーを停止しリンク先が実際に機能しなくなったプロダクト（例: `tasukiba` 系ページ）に使用
- **アプリ導線の固定ピル**: `url` を持ち `status` が `suspended` / `ended` 以外の **たすきば配下ページ**（slug が `tasukiba`、または `parent: "tasukiba"`）に限り、`url`（たすきばアプリ）へ飛ぶ小型の固定ピル（フクロウアイコン + ラベル、`position: fixed`、右下／モバイルは下部中央、本文を覆わない）を表示。連載ブログの固定ピル（1.6）と同一の挙動で、リンクは CSS で初期表示され JS 無効でもクロール可能、JS は ✕ での閉じ操作と `sessionStorage`（キー `appFloatDismissed:<slug>`）による閉状態の記憶のみを担う。ラベル等は i18n `product.appFloat`（`text` / `aria` / `dismissAria`）。たすきばは運用終了 (`status: ended`) のため現在は非表示
...
- **独立 LP への差し替え（`landingPage`）**: frontmatter に `landingPage`（`public/` 配下のルート相対パス）を持つプロダクトは、Home のプロダクトカード・製品一覧の「詳細を見る」ボタン・親ページの子カードのリンク先が、通常の Content Collections 詳細ページではなくそのパスへ差し替わる（3 箇所とも `src/i18n/content.ts` の `getProductDetailUrl` に統一）。Content Collections 側のページ自体は変わらず生成されるため、直リンクや検索流入は従来どおり到達可能。現状この設定を使用しているプロダクトはない（2026-09 に `tasukiba-user` が運用終了に伴い `landingPage: "products/tasukiba-lp/"` を解除し、通常の詳細ページに一本化）。`public/products/tasukiba-lp/index.html` は静的ファイルとして残置し、`tasukiba-user` 詳細ページおよび GitHub リポジトリへ誘導する「運用終了のお知らせ」ページ（5 秒後に自動リダイレクト、`noindex`）に置き換え済み。旧 LP への外部リンク・ブックマーク経由のアクセスを考慮した措置
